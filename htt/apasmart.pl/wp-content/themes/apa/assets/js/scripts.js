...
APA.vars = {
    // Selectors
    fullPageElem: $('.js-full-page'),
    mainMenu: $('.page-banner'),
    equalBox: $('.js-equal-height'),
    mainContent: $('.main'),
    textBox: $('.js-text-box'),
    subMenuToggle: $('.dropdown-toggle'),
    topArrow: $('.js-go-to-top'),
    mobileMenuTrigger: $('.js-nav-trigger'),
    mobileMenuClose: $('.js-nav-close'),
    sectionLink: $('.js-section-pagination'),
    cookieInfo: $('.js-cookie-info'),
    mailField: $('.wpcf7-email'),
    stellarContainer: $('.js-stellar'),
    boxHeight: $('.js-box-height'),
    chart: $('.js-chart'),
    contact: $('.js-contact'),
...
// Run on page init
APA.init = function() {
    if( APA.vars.fullPageElem.length ) {
        APA.fullPageSection(APA.vars.fullPageElem, APA.vars.mainMenu);
    }

    if( APA.vars.equalBox.length ) {
        APA.setBoxHeight(APA.vars.equalBox);
    }

    if( APA.vars.textBox.length ) {
        APA.setTextBoxHeight(APA.vars.textBox);
    }

    if( APA.vars.subMenuToggle.length ) {
        APA.openSubMenu(APA.vars.subMenuToggle);
    }

    if( APA.vars.topArrow.length ) {
        APA.goTop(APA.vars.topArrow);
    }

    if( APA.vars.sectionLink.length ) {
        APA.sectionPagination(APA.vars.sectionLink);
    }

    if( APA.vars.cookieInfo.length ) {
        APA.cookieInfo(APA.vars.cookieInfo);
    }

    if( APA.vars.stellarContainer.length ) {
        APA.stellarInit(APA.vars.stellarContainer);
    }

    if( APA.vars.boxHeight.length ) {
        APA.whiteBoxHeight(APA.vars.boxHeight);
    }

    if( APA.vars.chart.length ) {
        APA.chart(APA.vars.chart);
    }


    if( APA.vars.contact.length ) {
        APA.contact();
    }

    if( APA.vars.pagination.length ) {
        APA.pagination(APA.vars.pagination);
    }

    APA.mobileMenu(APA.vars.mobileMenuTrigger, APA.vars.mobileMenuClose);

    APA.clearFieldValue(APA.vars.mailField);
};
...
APA.resize = function() {
    ...
    APA.setBoxHeight(APA.vars.equalBox);
    APA.setTextBoxHeight(APA.vars.textBox);
    APA.helperWindowSize();
    APA.resetMenuVisibility();
    APA.stellarInit();
    APA.whiteBoxHeight(APA.vars.boxHeight);
};
...
APA.helperWindowSize = function() {
    APA.vars.windowHeight = window.innerHeight;
    APA.vars.windowWidth = window.innerWidth;
};

APA.resetMenuVisibility = function() {
    var menuItems = document.querySelectorAll('.nav--main > li'),
        menuItemsLength = menuItems.length,
        i = menuItemsLength - 1;

    if( APA.vars.windowWidth >= 1024 ) {
        for( i; i >= 0; i-- ) {
...
APA.fullPageSection = function(elem, mainMenu) {
    var windowHeight = APA.vars.windowHeight,
        windowWidth = APA.vars.windowWidth,
    ...
    if( windowWidth >= 767 ) {
        elemHeight = windowHeight - menuHeight;
    } else {
        elemTopPos = elem.offset().top;
        elemHeight = windowHeight - elemTopPos;
    }

    if( elemHeight < treshold ) {
        elemHeight = treshold;
    }
...
APA.setBoxHeight = function(elem) {
    APA.vars.mainContent.imagesLoaded(function() {
        var photoBox = elem.parent().next(),
            photoBoxHeight = photoBox.innerHeight(),
            marginTop = elem.parents('.js-set-margin-top'),
            container = elem.parent('.text-highlighted__container');

        container.css('padding-bottom', '');
        elem.css('height', '');

        if(!marginTop.length) {
            elem.css('height', photoBoxHeight);
        }
...
APA.setTextBoxHeight = function(elem) {
    var heightFunction = function(box){
            APA.vars.mainContent.imagesLoaded(function() {
                var sidePhotos = box.closest('.grid__item').prev('.grid__item'),
                    grid = box.closest('.grid'),
                    topPhotos = grid.prev('.grid'),
                    content = elem.children(),
                    gridParent = grid.parent(),
                    rightTextBox = gridParent.find('.js-set-margin-top'),
                    photoBox = grid.find('.box__photo--higher'),
                    textBox = grid.parent().find('.text-box--higher'),
                    windowWidth = APA.vars.windowWidth,
                    contentHeight,
                    sidePhotosHeight,
                    topPhotosHeight,
                    ...
                    gridHeight,
                    newGridHeight,
                    noSpace = gridParent.find('.js-no-space'),
                    rightBox = gridParent.find('.js-right-box'),
                    fixedBox = gridParent.find('.js-fixed');

                // Clear css data
                grid.css('height', '');
                box.css({'height' : '', 'top' : ''});
                rightTextBox.css('margin-top', '');
                textBox.css('margin-top', '');

                // Set position white box with text
                sidePhotosHeight = sidePhotos.outerHeight();
                topPhotosHeight = topPhotos.outerHeight();

                elemTopPos = -((topPhotosHeight*0.14) + sidePhotosHeight);

                contentHeight = content.outerHeight() + 135;
                newGridHeight = contentHeight - (topPhotosHeight*0.14);
                gridHeight = grid.outerHeight();

                // Set height white box with text
                if(windowWidth >= 1024) {

                    if(rightBox.length) {
                        box.css('height', contentHeight);
                        textBox.css('margin-top', - gridHeight + newGridHeight + 35);
                        photoBoxHeight = photoBox.innerHeight();
                        rightTextBox.css('margin-top', - (newGridHeight - photoBoxHeight));
                    }
                    else if(noSpace.length) {
                        textBox.css('margin-top', - 150);
                    }
                    else if(rightTextBox.length) {
                        textBox.css('margin-top', '');
                    }
                    else {
                        textBox.css('margin-top', - gridHeight + newGridHeight + 35);
                ...
                else if(windowWidth >= 767 && newGridHeight > gridHeight) {
                    box.css('height', contentHeight);
                    grid.css('height', newGridHeight);
                }

                // For section in need page
                if(fixedBox.length && windowWidth >= 767) {
                    box.css('top', -75);

                    if(newGridHeight > gridHeight + 35) {
                        box.css('height', contentHeight - 20);
                        grid.css('height', newGridHeight - 40);
                    }
                    else {
                        box.css('height', gridHeight + 75);
                    }

                }
                else {
                    box.css('top', elemTopPos);
                }
            });
        };
...
APA.openSubMenu = function(elem) {
    elem.on('click', function(e) {
        var subMenu = elem.next('.dropdown-menu');
...
APA.goTop = function($elem) {
    $elem.on('click', function(e) {
        e.preventDefault();

        TweenLite.to(window, 1.2, {scrollTo:{y:0}, ease:Sine.easeIn});
    });
...
APA.mobileMenu = function(openMenu, closeMenu) {
    var menuContainer = document.querySelector('.nav__container'),
        menuItems = document.querySelectorAll('.nav--main > li'),
        menuItemsLength = menuItems.length,
        bodyElement = document.querySelector('body'),
        closeButton = document.querySelector('.js-nav-close'),
        i;

    openMenu.on('click', function(e) {
        e.preventDefault();
        $('body').css('overflow','hidden');
        bodyElement.classList.toggle('nav__container--open');
        TweenLite.to(menuContainer, 0.75, {opacity: 1});

        TweenLite.to(closeButton, 0.35, {
            opacity: 1,
            y: 0,
            delay: 0.5
        });

        for( i = menuItemsLength - 1; i >= 0; i-- ) {
            TweenLite.to(menuItems[i], 0.25, {
                opacity: 1,
                x: 0,
                delay: (i*0.1),
                ease: Sine.easeIn
    ...
    closeMenu.on('click', function(e) {
        ...
        TweenLite.to(menuContainer, 0.75, {
            opacity: 0,
            delay: 0.75,
            onComplete: function() {
                bodyElement.classList.remove('nav__container--open');
            }
        });
...
APA.showHeaderLogo = function() {
    var headerLogo = document.querySelector('.logo--header'),
        scrollPos = $(window).scrollTop();

    if( scrollPos > 250 ) {
        TweenLite.to(headerLogo, 0.25, {opacity: 1});
        headerLogo.style.visibility = 'visible';
    } else {
        TweenLite.to(headerLogo, 0.25, {opacity: 0});
        headerLogo.style.visibility = 'hidden';
    }
};


APA.sectionPagination = function(elem) {
    elem.onePageNav({
        currentClass: 'active',
        changeHash: false,
        scrollSpeed: 750,
...
APA.clearFieldValue = function(elem) {
    var currentValue = elem.val(),
        defaultText = 'wpisz swój adres e-mail',
        newValue,
        clearVelue = function(value, elem) {
            newValue = elem.val();
            if( newValue === defaultText ) {
                elem.val('');
            }
        },
        restoreVelue = function(value, elem) {
            elem.val(value);
        };
    ...
    elem.on('focusout', function() {
        ...
        if( newValue === '' ) {
            restoreVelue(currentValue, $(this));
        }
    });
};


APA.cookieInfo = function(elem) {
        var $trigger = elem.find('.close-button'),
            cookie = APA.helpers.getCookie('apa-new-user');
        ...
        $trigger.on('click', function(e) {
            e.preventDefault();
            APA.helpers.setCookie('apa-new-user',1,500);
            elem.fadeOut('slow');
        });
};

APA.stellarInit = function(elem) {
    if( APA.vars.windowWidth > 1024 ) {
        $.stellar({
...
APA.whiteBoxHeight = function(elem) {
    var grid = elem.parent(),
    ...
    if(elemHeight < gridHeight && APA.vars.windowWidth >= 1024) {
        elem.css({height : gridHeight});
    }
};

APA.chart = function(elem) {
    var speedDesktop = [500, 1000, 1500, 2000, 2500, 3000, 3500];
    var speedTablet = [500, 750, 1000, 1250, 1500];

    var animation = function($this, speed, timeout) {
        var bars = $this.find('.bars li .bar');

        bars.each( function() {
            var $bar = $(this),
                percentage = $bar.data('percentage'),
                x = Math.floor((Math.random() * speed.length));
    ...
    $(window).scroll( function(){
        $('.chart').each( function(i){
            var $this = $(this),
                ...
                bottom_of_window = $(window).scrollTop() + $(window).height() + 50;

            if(APA.vars.windowWidth < 1024 && bottom_of_window + 100 > bottom_of_object) {
                timeout = 600;
                animation($this, speedTablet, timeout);
            }
            else if( bottom_of_window > bottom_of_object ){
                timeout = 1500;
                animation($this, speedDesktop, timeout);
            }
        });
    });
};
...
APA.contact = function() {
    var draw;
    $("#gform_browse_button_1_2").val("DODAJ PLIKI");

    if( document.createElement('svg').getAttributeNS ) {

        var checkbxsCheckmark = Array.prototype.slice.call( document.querySelectorAll( 'input[type=checkbox]' ) ),
            pathDefs = {
        ...
        var controlCheckbox = function( el, type, svgDef ) {
    ...
    draw(document.getElementById('choice_1_11_1'),'checkmark');

};

APA.pagination = function(elem) {
    $(function() {
...
$(window).on('resize', _.throttle(APA.resize, 100));

$(window).on('scroll', _.throttle(APA.scroll, 100));
